...
rom jinja2 import Environment, FileSystemLoader
mport numpy as np
import parmed
...
mport time
mport signac
rom flow import FlowProject, aggregator
rom flow.environment import DefaultSlurmEnvironment
mport flow
...
rom files.python_files import names
rom files.python_files import job_tester

 GMXPrefix mapping
MX_PREFIX = '/usr/local/gromacs/bin/gmx' # for potoff cluster

ef simple_mdp_writer(job, mdp_name, parameters, constraints=None, templates_dir=None, template_name=None):
...
ef write_gmxINDEX_forRESIDUES(job, top_file='init.top', gro_file='init.gro', index_file_name='whacky_index_file.ndx'):
   with(job):
...
ef manual_gmx_index_file_make(job, gro_file='init.gro', index_file_name='whacky_index_file.ndx', skip_residues_from_ncompounds=1000):
   with(job):
...
ef gmx_density_profile(job, trr_or_gro, index_file, tpr_file, output_xvg_name, first_frame, last_frame, slices=128):
   with(job):
...
ef give_name_return_whichChunk(job, chunk_dict):
   with(job):
       last_chunk = 0
...
ef calculate_free_energy(target_dir):
   import sys
   import os
   import subprocess
